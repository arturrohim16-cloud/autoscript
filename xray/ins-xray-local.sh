...
echo -e "[SERVICE] Restart All service"
systemctl daemon-reload
sleep 0.5
echo -e "[ ok ] Enable & restart xray "
systemctl daemon-reload
...
cd /usr/bin/
# Copy XRAY scripts from local repository
# vmess
cp "$BASEDIR/xray/add-ws.sh" add-ws && chmod +x add-ws
cp "$BASEDIR/xray/trialvmess.sh" trialvmess && chmod +x trialvmess
cp "$BASEDIR/xray/renew-ws.sh" renew-ws && chmod +x renew-ws
cp "$BASEDIR/xray/del-ws.sh" del-ws && chmod +x del-ws
cp "$BASEDIR/xray/cek-ws.sh" cek-ws && chmod +x cek-ws

# vless
cp "$BASEDIR/xray/add-vless.sh" add-vless && chmod +x add-vless
cp "$BASEDIR/xray/trialvless.sh" trialvless && chmod +x trialvless
cp "$BASEDIR/xray/renew-vless.sh" renew-vless && chmod +x renew-vless
cp "$BASEDIR/xray/del-vless.sh" del-vless && chmod +x del-vless
cp "$BASEDIR/xray/cek-vless.sh" cek-vless && chmod +x cek-vless

# trojan
cp "$BASEDIR/xray/add-tr.sh" add-tr && chmod +x add-tr
cp "$BASEDIR/xray/trialtrojan.sh" trialtrojan && chmod +x trialtrojan
cp "$BASEDIR/xray/del-tr.sh" del-tr && chmod +x del-tr
cp "$BASEDIR/xray/renew-tr.sh" renew-tr && chmod +x renew-tr
cp "$BASEDIR/xray/cek-tr.sh" cek-tr && chmod +x cek-tr

# shadowsocks
cp "$BASEDIR/xray/add-ssws.sh" add-ssws && chmod +x add-ssws
cp "$BASEDIR/xray/trialssws.sh" trialssws && chmod +x trialssws
cp "$BASEDIR/xray/del-ssws.sh" del-ssws && chmod +x del-ssws
cp "$BASEDIR/xray/renew-ssws.sh" renew-ssws && chmod +x renew-ssws
